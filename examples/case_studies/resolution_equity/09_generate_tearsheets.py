#!/usr/bin/env python3
"""Step 9: Render jellycell tearsheets from the analysis results.

Additive step — runs AFTER ``08_generate_findings.py`` and does NOT
...
Engines bridged (when present in ``analysis_results.json``):

- ``policy_evaluation`` (ITS) → ``did_results.json``
  (as a DiD-compatible level-change row)

Other nyc311 outputs (STL decomposition, Theil index, Oaxaca-Blinder
decomposition, spatial lag models, reporting-bias EM) remain in the
homegrown ``FINDINGS.md`` only, because factor-factory's tearsheet
templates don't have dedicated slots for every family.
"""
...
def run() -> None:
    ...
    if not ANALYSIS_JSON.exists():
        ...
        print("  Run step 8 ('08_generate_findings.py') first.")
        return

    MANUSCRIPTS_DIR.mkdir(exist_ok=True)
    analysis = json.loads(ANALYSIS_JSON.read_text())

    ensure_jellycell_toml(HERE, "resolution-equity-study")
