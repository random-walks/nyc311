...
- The cached slice contains `15000` complaint records sourced from `cache/topic-eda-snapshot.csv` (`live fetch`).
- The weakest built-in topic coverage in this slice is `HEAT/HOT WATER` at `0.0%` matched.
- The strongest built-in topic coverage is `Noise - Residential` at `92.9%` matched.
- The biggest unmatched descriptor driver is `ENTIRE BUILDING` with `1618` unmatched rows in the coverage audit.
- The largest anomaly by absolute z-score is `BROOKLYN 03 / Party Music` at `+3.28`.
- The highest unresolved share in the cached slice appears in `Construction Lead Dust` at `100.0%` unresolved.
- In the synthetic Water System demo, custom rules change coverage from `100.0%` to `66.7%`.
