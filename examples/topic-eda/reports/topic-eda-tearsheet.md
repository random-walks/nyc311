...
- The cached slice contains `5000` complaint records sourced from `cache/topic-eda-snapshot.csv` (`cache`).
- The weakest built-in topic coverage in this slice is `HEAT/HOT WATER` at `0.0%` matched.
- The strongest built-in topic coverage is `Noise - Residential` at `94.3%` matched.
- The biggest unmatched descriptor driver is `ENTIRE BUILDING` with `370` unmatched rows in the coverage audit.
- The largest anomaly by absolute z-score is `03 BROOKLYN / Party Music` at `+2.71`.
